...
for g in target_grades:
    ...
    for n in indices:
        ...
        for control_token in ['[V1]', '[V2]', '[V3]', '[V4]']:
            input_ids = tokenizer(f"{prefix} {sent} {control_token}", return_tensors="pt", max_length=128, truncation=True).input_ids
            ...
            print("PREDICTION:\t", control_token, decoded_preds, '\n')

            # Rouge expects a newline after each sentence
            decoded_preds = ["\n".join(nltk.sent_tokenize(pred.strip())) for pred in [decoded_preds]]
            decoded_labels_str = ["\n".join(nltk.sent_tokenize(label.strip())) for label in [decoded_labels]]

            result = metric.compute(predictions=decoded_preds, references=decoded_labels_str, use_stemmer=True)
